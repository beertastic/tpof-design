---
title: "Baylan — Movement Breakdown, Scenes 1–20"
asset_id: "BLOCKING-BAYLAN-ACT-ONE"
version: "1.0"
status: "working — all five blocking questions answered 2026-08-01"
source: "Screenplay v10"
---

# Baylan — Movement Breakdown, Scenes 1–20

Every move he makes from the first frame until the crew heads out, with who he is
with and where. Derived from v10, not from memory. Built to hang the virtual map
on and then to write the script directions back from.

**Nine of the first twenty scenes are his.** He is in more of this stretch than
anyone, and he is alone in most of it.

---

## The spaces, as the script currently implies them

| # | Space | Scenes | Int/Ext |
|---|---|---|---|
| **A** | **The meditation clearing** — "a small clearing" | 3, 5, 7 | Ext |
| **B1** | **Jasu's nook** — "a small, cramped clearing" at the treeline, under a low branch, tarpaulin against a tree | 9 | Ext |
| **B2** | **The open camp** — the landing clearing, the ship, the fire, the stores | 19 | Ext |
| **C** | **The ship — main hold** | 10 | Int |
| **D** | **The ship — crew bunks** | 13, 15 | Int |
| **E** | **The storage area** — "makeshift storage area, surrounded by crates and spare ammo boxes" | 14 | Ext |

**Settled 2026-08-01.** The mercenaries could not overfly the dampening field or
set down at the wreck, so they landed at **the first clearing that would hold a
YT-2000**, out on the outskirts of the dense forest, about two kilometres from
the crash. That clearing is big.

Which means **Sc.9's "small, cramped clearing" is not the camp** — it is Jasu's
nook at the treeline, and no line has to change. The set has two registers and
the scenes divide cleanly between them:

| Register | Character | Scenes |
|---|---|---|
| **Open, at the ship** | Room for ten people, sky overhead, hull behind them | 19 |
| **Tight, at the treeline** | Nooks, low branches, no sky, a man can be unobserved | 9, 14 |

**The ramp faces the treeline, not the fire** — Sc.13 runs ramp → trees →
storage in one move, so he gets into cover without crossing open ground or
passing anyone.

See [`07-locations/forest-mercenary-camp/`](../../07-locations/forest-mercenary-camp/)
and [`07-locations/forest-world/`](../../07-locations/forest-world/).

---

## The sequence

### Sc.3 — EXT. FOREST. Evening. **Space A. Alone.**

Sitting cross-legged in a small clearing, palms on knees, eyes shut hard. Not
relaxed — flinching at remembered screams and lightsabers.

**Blocking:** static, seated, low. The costume has to work folded.
**Note:** the crew is unloading the ship at this moment (Sc.7 dialogue), so he has
deliberately walked away from work to do this.

### Sc.5 — EXT. FOREST. Evening. **Space A. Alone.**

Same position, eyes still closed, head tilting. He is hunting a new sound.

### Sc.7 — EXT. FOREST. Evening. **Space A. With NYX.**

Eyes snap open. Nyx is **crouched in front of him, hand on his shoulder** — so
Nyx has walked in and got down to his level while he was under.

| Beat | Movement |
|---|---|
| Nyx wakes him | Crouched, contact on the shoulder |
| *"we're unloading the ship. Captain wants us to make camp"* | — |
| Nyx slaps his shoulder, walks away, hefts a crate | Nyx exits toward the unloading |
| Bay gets to his feet | First time he stands in the film |
| **He approaches the edge of the clearing** | Ends facing out, into the trees |

**Contact:** Nyx touches him twice in one scene, both times on the shoulder. He is
the only person in the film who touches Baylan.

### Sc.9 — EXT. MERCENARY CAMP. Evening. **Space B1, Jasu's nook. With JASU, the akk dog, then the whole crew.**

> Bay **ducks under a particularly low branch** into a small, cramped clearing.

| Beat | Movement | Note |
|---|---|---|
| Enters under a low branch | Has to stoop. He is tall and broad — make the branch do that work | |
| Jasu is under a tarpaulin leaned against a tree, the akk dog curled beside her | She does not look up | |
| **Bay kneels beside her** | Down to her level, unbidden | **He kneels here, and refuses to in Sc.19.** Same body, opposite meaning |
| Tatooine exchange | Shared history, easy | The only time he is warm with anyone but Nyx |
| **He and the beast lock eyes. It gets up and lies beside Bay instead** | The animal crosses from Jasu to him | **Plant.** This is why it obeys him in Sc.25 |
| *"I'm still getting sand out of places I didn't know existed"* | | |
| **He breaks eye contact and shifts where he sits** | The tell. He is having doubts | |
| Jasu calls "Gather round!", whistles, the dog returns to her | The crew arrive **through the trees** | |
| **The crew circle the captain and kneel** | Bay is already down | Does he stay kneeling, or stand as they arrive? |
| Briefing, dismissal, they disperse | | |
| **Bay looks out into the darkening forest, a crease forming** | Ends facing away from the camp | Rhymes with Sc.7's ending |

### Sc.10 — INT. SHIP, MAIN HOLD. Night. **BAYLAN IS ABSENT — for now.**

The Sabacc game. Nyx, Shada, Reya Fenn, Yaslo Bis and Mercs 1–4. **Jasu is absent
too**, and the crew gossips about both of them in consecutive lines.

**Decided 2026-08-01: he stays out, and the lurking option is withdrawn.** The
scene has been reassigned as the mercenary cast's showcase, and he is the
point-of-view character — an audience nine scenes into following him will watch
him rather than the crew. See
[`Sc10-Sabacc-Showcase.md`](Sc10-Sabacc-Showcase.md).

### Sc.13 — INT. SHIP, CREW BUNKS. Night. **Space D. Alone, among sleepers.**

Wakes from a bed roll, drenched, breathing ragged.

| Beat | Movement |
|---|---|
| Shoots up from the bed roll | |
| Runs a hand through his hair | |
| **Peers around at the other bunks** — nobody has stirred | He checks whether he was heard. Eighteen years of that |
| Pushes himself up | |
| **Slips down the ramp into the crisp night air** | Straight into Sc.14 |

### Sc.14 — EXT. FOREST, MOMENTS LATER. **Space E. Alone, then NYX.**

> Bay **trudges through the trees**… **drops onto a rock**… He's found himself in
> the makeshift storage area.

**"Found himself" is the key phrase — he did not walk there on purpose.** He was
trying to re-enter the vision and ended up somewhere.

| Beat | Movement | Note |
|---|---|---|
| Trudges through the trees | Aimless | |
| Closes his eyes, tries to re-enter the vision | Hears only Shin's scream | |
| Drops onto a rock | Seated again | Third time seated in the act |
| Takes out the blaster, looks at it | *"The barrel resembles a lightsaber's handle…"* | |
| **Focuses on a hefty supply box. It rises shakily** | Standing or seated? Unspecified | The first Force use in the film |
| **Nyx appears from the trees beside him**, rifle in hands | Bay jumps, the box thumps down | Nyx is on patrol — he has a route |
| Bay shoves the blaster back in the holster **and stands** | Covering | |
| Shoulder push, *"Can't keep our Bay from a good fight, eh?"* | Contact again | |
| They part | **Bay heads back toward the camp** | |

**This scene only works because there are no tents.** He has nowhere private to
go, so he walks out into the trees — and that is exactly what puts him in front of
a man on patrol.

### Sc.15 — INT. SHIP, CREW BUNKS. Later. **Space D. Alone, among sleepers.**

Cross-legged on his bunk, the others asleep around him, lit by a glowrod.

Cord, pouch, crystal, the blaster comes apart.

**The most private thing he does in the film happens in a room with four other
people in it.** That is the whole value of moving it aboard.

### Sc.19 — EXT. MERCENARY CAMP. Early morning. **Space B2, open by the ship. Whole crew.**

| Beat | Movement |
|---|---|
| The crew form a **semi-circle** around Jasu | |
| **Those in front take a knee** | |
| **Bay stands towards the back, arms crossed, jaw tense** | He does not kneel |
| *"we're paid, breathing or not"* — chuckles ripple | **Bay stays stonily silent** |
| *"No matter the cost!"* | Does he say it? |
| They stand, slap each other's backs, ready gear | |

### Sc.20 — EXT. FOREST. Early morning. **On the move. NYX, JASU, the crew.**

Rifle slung, no blaster on his hip — Sc.15 emptied the holster and Nyx notices.
Jasu hands him the scanner. He finds them, and his jaw tightens.

---

## What the map has to support

1. **A stoop-height branch** into Jasu's nook at the treeline.
2. **A tarpaulin leaned against a tree**, big enough for Jasu and a large animal,
   with room for a second person to kneel.
3. **Space for ten people to form a semi-circle** with several kneeling (Sc.19),
   in the open by the ship — a different part of the set from Sc.9's nook.
4. **A ramp** that puts a man straight into trees.
5. **A stack of crates and ammunition boxes** with a sittable rock, far enough
   from the ship to feel unobserved and on somebody's patrol route.
6. **A fire and a tarp windbreak** — the camp is a working area, nobody sleeps
   out here.

---

## Questions — all five answered

### 1. Where does the ship sit, and how big is the camp? — **ANSWERED 2026-08-01**

**The landing clearing is big.** They could not overfly the dampening field or
set down at the wreck, so they went out to the outskirts of the dense forest and
took the first clearing that would hold a YT-2000, about two kilometres from the
crash.

**And Scene 9 needs no change.** *"A small, cramped clearing"* under *"a
particularly low branch"* is **Jasu's nook at the treeline**, not the main camp —
she has taken a pocket at the edge for herself and the animal, and the crew comes
to her. Sc.9 has her call *"Gather round!"* **through the trees**, and they crowd
in and kneel around her in the dirt.

So the set has two registers: **open at the ship** (Sc.19, ten people, sky) and
**tight at the treeline** (Sc.9 and 14, no sky, a person can be unobserved).

### 2. Meditation clearing and camp — two spaces or one? — **TWO**

Space A is out at the forest edge, away from the unloading. He has walked off to
be by himself, which is the first thing the film tells us about him and the same
thing Sc.14 tells us again.

### 3. Where is Baylan during the Sabacc game? — **ANSWERED 2026-08-01: out, for now**

He stays out of Sc.10. **But he has no dialogue in it, so nothing stops him being
there** — and the option is live: lurking at the edge of the lamp light, silent,
chuckling at the remarks made about him.

That is worth keeping on the table, because **Jasu is absent too, and both of them
get talked about in consecutive lines:**

> **SHADA:** Can't be upsetting Jasu's golden boy!
>
> **NYX:** Oh can it. We all know Bay's her right hand.

**The two people not at the card game are the captain and the one who is hiding.**
Jasu is apart by rank; Baylan by secret. Neither can afford to be one of the crew,
for opposite reasons.

If he does lurk, Nyx's line stops being gossip and becomes something said *to*
him — needling, affectionate, and completely wrong about why he is Jasu's right
hand. That is a good beat, it needs no dialogue, and it costs one more body on
the hold set.

**Consequence for the roster: Sc.10 is eight, not ten.** Corrected.

### 4. How far is the storage area from the ship? — **ANSWERED**

At the treeline, past the fire and past the ramp light. Far enough to be in the
dark and out of casual sight, close enough for Nyx to walk into on a patrol
route and for Baylan to head *"back towards the camp"* in one cut.

### 5. Which way does the ramp face? — **THE TREELINE**

Scene 13 runs ramp → trees → storage in one move, so he gets off the ship into
cover **without crossing the open ground or passing the fire.** That is the
difference between a man slipping out and a man being seen to leave.

## Revision History

| Version | Date | Notes |
|---|---|---|
| 1.0 | 2026-08-01 | Built from v10 to support the virtual set. Five questions raised. |
