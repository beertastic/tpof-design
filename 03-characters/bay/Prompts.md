...
**Faction:** Mercenaries  
**Scenes:** 3, 5, 7, 9, 13, 14, 15, 19, 20  
**Primary environment:** Forest  
**Brief:** Point-of-view character carried through the mercenary camp and pursuit; highest scene count in the film.

---

## How to use

Each numbered slot is a complete, self-contained prompt. Paste **Style**, **Do Not**, **Character Constants**, then a single slot. Save the result to the exact filename given, in `03-characters/bay/source/artwork/`.
...
Bay. Point-of-view character carried through the mercenary camp and pursuit; highest scene count in the film.
...
> The Shada lock carries a `Design Drift Prevention` list — the specific wrong directions a generator will drift toward for *this* character. Write the equivalent for Bay. It is the single most useful part of the pack.
...
Waist-up portrait of Bay in forest, three-quarter angle, engaged in the moment rather than posing. Neutral working expression.
...
Full-figure shot of Bay in evening and night forest, mercenary camp, in motion or mid-task, read at the scale of the location around them.
...
Bay in a cramped, lived-in starship corridor — exposed pipework, access panels, mismatched paint, service markings, scuffed decking. Lit by bulkhead fixtures and warning lights.
...
Bay in a cramped shared compartment among their group, lit by work lamp and screen glow. Salvaged fittings, crates, worn webbing, visible repairs.
...
Bay in a dim hold or storage space, full body visible, silhouette clear. Stacked salvage, tie-downs, fuel staining, repaired plating. Single overhead work lamp with strong falloff, blacks kept open.
...
Close portrait of Bay, head and shoulders, lit at a grazing angle to reveal skin and material texture. Practical make-up and prosthetic quality, not digital fantasy. Soft overcast daylight, no costume detail competing.
...
Overhead flat-lay of Bay's complete carried kit on a worn surface, laid out in the order that person would keep it. Every item shows use and repair. Even, flat reference lighting.
...
Bay absorbed in an ordinary, unhurried task that reveals character — repairing, checking, treating, preparing. Not looking up. Warm practical lamp light against a dim salvaged interior. Hands prominent.
...
All land in `03-characters/bay/source/artwork/`. The board generator places images with a **contain** operation and never crops — supplying the wrong ratio produces letterboxing on the board.

This character has no `board-data.yaml` yet, so `generate.py bay` will not run. Create one modelled on `03-characters/shada/board-data.yaml`, using the filenames above.
...
- Bay has no entry in `Character.md` yet — this folder was created with the prompt pack. A `Character.md` and `Character-Lock.md` must be authored before these prompts can be completed.
- Open question: are Bay and Baylan the same character? Both are named in Scene 24A. Resolve before generating images for either.
