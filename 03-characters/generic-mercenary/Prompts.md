...
**Faction:** Mercenaries  
**Scenes:** 7, 9, 10, 19, 20, 21, 23  
**Primary environment:** Forest Mercenary Camp  
**Brief:** Reusable background and supporting mercenary design family.

---

## How to use

Each numbered slot is a complete, self-contained prompt. Paste **Style**, **Do Not**, **Character Constants**, then a single slot. Save the result to the exact filename given, in `03-characters/generic-mercenary/source/artwork/`.
...
Generic Mercenary. Reusable background and supporting mercenary design family.
...
> The Shada lock carries a `Design Drift Prevention` list — the specific wrong directions a generator will drift toward for *this* character. Write the equivalent for Generic Mercenary. It is the single most useful part of the pack.
...
Waist-up portrait of Generic Mercenary in forest mercenary camp, three-quarter angle, engaged in the moment rather than posing. Neutral working expression.
...
Full-figure shot of Generic Mercenary in camp, patrol, pursuit across all times of day, in motion or mid-task, read at the scale of the location around them.
...
Generic Mercenary in a cramped, lived-in starship corridor — exposed pipework, access panels, mismatched paint, service markings, scuffed decking. Lit by bulkhead fixtures and warning lights.
...
Generic Mercenary in a cramped shared compartment among their group, lit by work lamp and screen glow. Salvaged fittings, crates, worn webbing, visible repairs.
...
Generic Mercenary in a dim hold or storage space, full body visible, silhouette clear. Stacked salvage, tie-downs, fuel staining, repaired plating. Single overhead work lamp with strong falloff, blacks kept open.
...
Close portrait of Generic Mercenary, head and shoulders, lit at a grazing angle to reveal skin and material texture. Practical make-up and prosthetic quality, not digital fantasy. Soft overcast daylight, no costume detail competing.
...
Overhead flat-lay of Generic Mercenary's complete carried kit on a worn surface, laid out in the order that person would keep it. Every item shows use and repair. Even, flat reference lighting.
...
Generic Mercenary absorbed in an ordinary, unhurried task that reveals character — repairing, checking, treating, preparing. Not looking up. Warm practical lamp light against a dim salvaged interior. Hands prominent.
...
All land in `03-characters/generic-mercenary/source/artwork/`. The board generator places images with a **contain** operation and never crops — supplying the wrong ratio produces letterboxing on the board.

This character has no `board-data.yaml` yet, so `generate.py generic-mercenary` will not run. Create one modelled on `03-characters/shada/board-data.yaml`, using the filenames above.
...
- This is a design *family*, not one person. The prompt pack should produce a modular kit — shared base costume plus swappable elements — so background figures vary without breaking faction identity.
- Generate variation sheets rather than single hero images for most slots.
