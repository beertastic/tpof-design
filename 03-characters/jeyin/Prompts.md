...
**Faction:** Slaves / Escapees  
**Scenes:** 6, 8, 11, 16, 17, 18, 21, 22  
**Primary environment:** Forest  
**Brief:** Injured parent protecting Shin during the escape.

---

## How to use

Each numbered slot is a complete, self-contained prompt. Paste **Style**, **Do Not**, **Character Constants**, then a single slot. Save the result to the exact filename given, in `03-characters/jeyin/source/artwork/`.
...
Jeyin. Injured parent protecting Shin during the escape.
...
> The Shada lock carries a `Design Drift Prevention` list — the specific wrong directions a generator will drift toward for *this* character. Write the equivalent for Jeyin. It is the single most useful part of the pack.
...
Waist-up portrait of Jeyin in forest, three-quarter angle, engaged in the moment rather than posing. Neutral working expression.
...
Full-figure shot of Jeyin in forest flight, escapee camp, forest ditch, in motion or mid-task, read at the scale of the location around them.
...
Jeyin in a cramped, lived-in starship corridor — exposed pipework, access panels, mismatched paint, service markings, scuffed decking. Lit by bulkhead fixtures and warning lights.
...
Jeyin in a cramped shared compartment among their group, lit by work lamp and screen glow. Salvaged fittings, crates, worn webbing, visible repairs.
...
Jeyin in a dim hold or storage space, full body visible, silhouette clear. Stacked salvage, tie-downs, fuel staining, repaired plating. Single overhead work lamp with strong falloff, blacks kept open.
...
Close portrait of Jeyin, head and shoulders, lit at a grazing angle to reveal skin and material texture. Practical make-up and prosthetic quality, not digital fantasy. Soft overcast daylight, no costume detail competing.
...
Overhead flat-lay of Jeyin's complete carried kit on a worn surface, laid out in the order that person would keep it. Every item shows use and repair. Even, flat reference lighting.
...
Jeyin absorbed in an ordinary, unhurried task that reveals character — repairing, checking, treating, preparing. Not looking up. Warm practical lamp light against a dim salvaged interior. Hands prominent.
...
All land in `03-characters/jeyin/source/artwork/`. The board generator places images with a **contain** operation and never crops — supplying the wrong ratio produces letterboxing on the board.

This character has no `board-data.yaml` yet, so `generate.py jeyin` will not run. Create one modelled on `03-characters/shada/board-data.yaml`, using the filenames above.
...
- Jeyin is injured across the run of the story. Decide when the injury happens and track its visible state per scene — this is a continuity requirement, not a design flourish.
