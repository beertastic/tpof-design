---
title: "Krellis — Image Prompt Pack"
...
**Faction:** Slaves / Escapees  
**Scenes:** 8, 16, 17, 18, 21, 22  
**Primary environment:** Forest  
**Brief:** Medic among the escapees; involved in the final confrontation.

---

## How to use

Each numbered slot is a complete, self-contained prompt. Paste **Style**, **Do Not**, **Character Constants**, then a single slot. Save the result to the exact filename given, in `03-characters/krellis/source/artwork/`.
...
Krellis. Medic among the escapees; involved in the final confrontation.
...
> The Shada lock carries a `Design Drift Prevention` list — the specific wrong directions a generator will drift toward for *this* character. Write the equivalent for Krellis. It is the single most useful part of the pack.
...
Waist-up portrait of Krellis in forest, three-quarter angle, engaged in the moment rather than posing. Neutral working expression.
...
Full-figure shot of Krellis in forest flight, forest ditch, final clearing, in motion or mid-task, read at the scale of the location around them.
...
Krellis in a cramped, lived-in starship corridor — exposed pipework, access panels, mismatched paint, service markings, scuffed decking. Lit by bulkhead fixtures and warning lights.
...
Krellis in a cramped shared compartment among their group, lit by work lamp and screen glow. Salvaged fittings, crates, worn webbing, visible repairs.
...
Krellis in a dim hold or storage space, full body visible, silhouette clear. Stacked salvage, tie-downs, fuel staining, repaired plating. Single overhead work lamp with strong falloff, blacks kept open.
...
Close portrait of Krellis, head and shoulders, lit at a grazing angle to reveal skin and material texture. Practical make-up and prosthetic quality, not digital fantasy. Soft overcast daylight, no costume detail competing.
...
Overhead flat-lay of Krellis's complete carried kit on a worn surface, laid out in the order that person would keep it. Every item shows use and repair. Even, flat reference lighting.
...
Krellis absorbed in an ordinary, unhurried task that reveals character — repairing, checking, treating, preparing. Not looking up. Warm practical lamp light against a dim salvaged interior. Hands prominent.
...
All land in `03-characters/krellis/source/artwork/`. The board generator places images with a **contain** operation and never crops — supplying the wrong ratio produces letterboxing on the board.

This character has no `board-data.yaml` yet, so `generate.py krellis` will not run. Create one modelled on `03-characters/shada/board-data.yaml`, using the filenames above.
...
- Occupation must be readable from silhouette per the Bible. A medic's kit is the clearest tell the escapees have — decide what was salvaged or hidden, and what it is carried in.
