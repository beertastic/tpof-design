...
leather-and-fabric backing and set **edge to edge in a honeycomb**, each plate
separate, with a narrow and even gap between it and its neighbours.
...
hexagons meeting edge to edge, like a honeycomb. The flex comes from the gaps
and the lacing between plates.
...
two fingers, laced onto a leather backing and set edge to edge in a honeycomb
with a narrow even gap between neighbours, filling the frame.
