...
leather-and-fabric backing and laid so that **each row laps the top edge of the
row below by about a third**. Overlapped, never tiled flat edge to edge.

**EVERY PLATE IS A HEXAGON WITH SIX STRAIGHT SIDES.** Not round, not a coin, not
a teardrop, not a fish scale. The overlap is what makes it read as scale — the
