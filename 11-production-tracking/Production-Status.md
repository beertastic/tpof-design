...
| **Drive publishing** | **Signed-off turnarounds go to Drive, one folder per character — `Drive-Publishing.md`.** The repo is the source of truth and Drive is overwritten. **Currently WRONG: Jasu's Drive folder holds the superseded v1 turnarounds** — ankle boots and a belt whistle. Needs `rclone` installed, then `./tools/publish-to-drive --go`. Check daily |
