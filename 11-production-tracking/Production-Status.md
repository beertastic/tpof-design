...
| **Drive publishing** | **Signed-off images and finished boards go to Drive, one folder per character — `Drive-Publishing.md`.** Three subfolders each: `turnarounds/`, `artwork/`, `boards/`. The repo is the source of truth and Drive is overwritten. **Currently WRONG: Jasu's Drive folder holds the superseded v1 turnarounds** — ankle boots and a belt whistle — **loose at the folder root, where they will shadow the correct plates unless purged**. Needs `rclone` installed, then `./tools/publish-to-drive captain-jasu --go --purge-shadows`. `./tools/check-drive` daily, by cron |
