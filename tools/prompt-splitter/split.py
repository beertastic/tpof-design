...
def build(character: str, slot: dict, blocks: dict, cap: set[int], anti: set[int],
    ...
    if not gate:
        pass
    # The guard, added 2026-08-03. This file and its short counterpart share a
    # filename and differ only by directory, and on that day a run of plates was
    # generated by pasting these instead. They are 5x to 8x over the working
    # budget, so the host discards the overflow silently, from the middle, and
    # nothing reports it: the taupe leather colour and the serpentine grain in
    # the cloth were both present in these files and neither reached the
    # generator. Two images were then debugged as missing requirements when the
    # requirements were there all along.
    #
    # It goes ABOVE the slot header because the failure is someone opening the
    # file and pasting from the top. A note further down is a note nobody reads.
    parts = [
        "!" * 74,
        "!!  THIS IS THE LONG SPECIFICATION FILE. DO NOT PASTE IT INTO A",
        "!!  GENERATOR. It is 5-8x over the working budget; the overflow is",
        "!!  dropped silently, from the middle, and nothing tells you.",
        "!!",
        f"!!  PASTE THIS INSTEAD:  prompts/slots-short/{slot['n']:02d}-"
        f"{Path(slot['file']).stem}.txt",
        "!!",
        "!!  The one you want says (short) on its Prompt version line.",
        "!!  This file is the specification, for reading and editing.",
        "!" * 74,
        "",
